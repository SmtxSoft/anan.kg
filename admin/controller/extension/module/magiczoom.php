...
require_once (DIR_APPLICATION . 'controller/'.$modulesPath.'/magiczoom-opencart-module/module.php');
...
class ControllerModuleMagicZoom extends Controller {
    ...
    public function index () {
            ...
            $this->language->load($modulesPath.'/magiczoom'); // load lang. file
            ...
            if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validate())) {
                if(isset($this->request->post['clear_cache']) && $this->request->post['clear_cache'] == '1') {
                    ...
                    require_once(DIR_APPLICATION . 'controller/'.$modulesPath.'/magiczoom-opencart-module/magictoolbox.imagehelper.class.php');
                    ...
                    $finalSettings = array('magiczoom_settings' => $this->request->post); //serialize to avoid module name check for each option
                    
                    if ($this->request->post['magiczoom_status'] == '1' && $this->request->post['magiczoom_status'] != $this->request->post['magiczoom_status_was']) { //fire only on activaion
                        $this->session->data['refresh_modifications'] = true;
                    }
                    
                    $this->model_setting_setting->editSetting('magiczoom', $finalSettings);
                    $this->params = $this->model_setting_setting->getSetting('magiczoom');//load settings from DB
                    ...
                    if (version_compare(VERSION,'2','<')) {
                        ...
                        $this->response->redirect($this->url->link($modulesPath.'/magiczoom', $tokenUrl, true));
                    }
                }
            }
	    ...
	    if (isset($this->params['magiczoom_settings'])) {
		  $this->params = $this->params['magiczoom_settings'];
	    } else {
		  $this->params = array();
	    }
		...
		foreach ($this->blocks as $profile_id => $profile_name) {
            ...
            $imagehelper = new MagicToolboxImageHelperClass($shop_dir, $pathToCache, $tool->params);
            $usedSubCache = $imagehelper->getOptionsHash();
            $cacheInfo = $this->MagicZoomgetCacheInfo($shop_dir.$pathToCache, $usedSubCache);
            $extension_data['path_to_cache'] = $pathToCache;
            $extension_data['total_items'] = $cacheInfo['totalCount'].' ('.$this->MagicZoomformat_size($cacheInfo['totalSize']).')';
            $extension_data['unused_items'] = $cacheInfo['unusedCount'].' ('.$this->MagicZoomformat_size($cacheInfo['unusedSize']).')';
	    ...
	    foreach ($profiles as $profile) {
	      $extension_data['parameters'][$profile] = $tool->params->getParams($profile); // LOAD PARAMS
            ...
            $extension_data['breadcrumbs'][] = array(
            'href'      => HTTPS_SERVER . 'index.php?route='.$modulesPath.'/magiczoom' . '&'.$tokenUrl,
            'text'      => $this->language->get('heading_title'),
            'separator' => ' :: '
            );

            $extension_data['action'] = HTTPS_SERVER . 'index.php?route='.$modulesPath.'/magiczoom' . '&'.$tokenUrl;
            ...
            if (isset($this->request->post['magiczoom_status'])) {
                $extension_data['magiczoom_status'] = $this->request->post['magiczoom_status'];
            }  else if (isset($this->request->post['magiczoom_settings']['magiczoom_status'])) {
                $extension_data['magiczoom_status'] = $this->request->post['magiczoom_settings']['magiczoom_status']; 
            } else {
                if (isset($this->params['magiczoom_status'])) {
                    $this->config->set('magiczoom_status',$this->params['magiczoom_status']);
                } 
                $extension_data['magiczoom_status'] = $this->config->get('magiczoom_status');
            }
            ...
            $data['groupsHTML']['general'] = $this->MagicZoom_groupsHTML($extension_data['parameters'],'default', $default_map);
            ...
            foreach ($this->blocks as $block_id => $block_name) { 
                ...
                $data['groupsHTML'][$block_id] .= $this->MagicZoom_groupsHTML($extension_data['parameters'], $block_id, $this->map);
                $data['groupsHTML'][$block_id] .= '</div>';

            }


            if (!preg_match('/DEMO/s',@file_get_contents(DIR_CATALOG.'view/javascript/magiczoom.js'))) {
                $data['trial'] = false; 
            } else {
                $data['trial'] = true;  
            }
            ...
            $data['trial_bage'] = '<div style="border: 1px solid #C7C16C;background: #feffd0;text-align: center;">
                                ...
                                To remove the red "<span style="color:red;">Please upgrade..</span>" text, please <a target="_blank" href="http://magictoolbox.com/buy/magiczoom/"><b>buy</b></a> and upload your licensed magiczoom.js file to this folder:
                                <div style="padding:10px 0;">'.preg_replace('/^.*?([^\/]*\/?$)/is','$1',DIR_CATALOG).'view/javascript/magiczoom.js</div></div>';
		...
		$this->template = $modulesPath.'/magiczoom_oc15.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
		...
		$data['footer'] = $this->load->controller('common/footer');
                if (version_compare(VERSION,'3','>=')) {
                    $this->response->setOutput($this->load->view($modulesPath.'/magiczoom', $data));
                } else {
                    $this->response->setOutput($this->load->view($modulesPath.'/magiczoom_oc2.tpl', $data));
                }
	    }
    ...
    private function validate() {
            ...
            if (!$this->user->hasPermission('modify', $modulesPath.'/magiczoom')) {
    ...
    function MagicZoomgetCacheInfo($path, $usedSubCache = null) {
        ...
        if ($handle = opendir($path)) {
            while (false !== ($file = readdir($handle))) {
                ...
                if ($file != '.' && $file != '..' && !is_link($next)) {
                    if (is_dir($next)) {
                        $result = $this->MagicZoomgetCacheInfo($next);
    ...
    function MagicZoomformat_size($size) {
        $units = array(' bytes', ' KB', ' MB', ' GB', ' TB');
        for ($i = 0; $size >= 1024 && $i < 4; $i++) $size /= 1024;
        return round($size, 2).$units[$i];
    }

    function MagicZoom_groupsHTML ($parameters, $profile = 'default', $map) {
        ...
        global $toolAbr, $magiczoom_status;
        ...
        foreach ($parameters as $name => $s) {
            ...
            if ($s["type"]=='num' || $s["type"]=='text') {
                $html  .= '<label class="col-sm-2 control-label" for="magiczoomsettings'. ucwords(strtolower($name)).'">'.$before.'</label>';
            } else {
            ...
            switch($s["type"]) {
                case "array":
                    ...
                    foreach($s["values"] as $p) {
                        ...
                        $rButtons[strtolower($p)] = '<label class="radio-inline"><input type="radio" value="'.$p.'"'. ($s["value"]==$p?"checked=\"checked\"":"").' name="'.$profile.'_'.$s['id'].'" id="magiczoomsettings'. ucwords(strtolower($name)).$p.'">';
                        $pName = ucwords($p);
                ...
                default:
                    ...
                    if (!isset($s["value"])) $s["value"] = $s["default"];
                    $html .= '<input class="form-control" '.$width.' type="text" name="'.$profile.'_'.$s['id'].'" id="magiczoomsettings'. ucwords(strtolower($name)).'" value="'.$s["value"].'" />';
                    break;
            }
...
if (version_compare(VERSION,'2.3','>=')) { //newer than 2.2.x
    class_alias('ControllerModuleMagicZoom','ControllerExtensionModuleMagicZoom'); //hack the class system
}
