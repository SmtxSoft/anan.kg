	...
	if ($tool->params->profileExists($type)) { //for main profiles
	    ...
	    if ($type == 'product' && $tool->type == 'standard') {
                ...
                if (isset($m360[1])) {
                    ...
                    if (isset($GLOBALS['magictoolbox']['magic360']->params)) {
                        $defaultView = strtolower($GLOBALS['magictoolbox']['magic360']->params->getValue('default-spin-view','product'));
                    } else {
                        $defaultView = 'spin';
                    }

                    
                    if ($defaultView == 'spin') { //show  spin 
                        ...
                        $content = preg_replace('/(<a[^>]*?id=\"Magic[^\"]*Image\d+_product\"[^>]*?class=\"(?:MagicZoom|MagicZoomPlus)\")/is','<div class="m360container" style="display: none;">'.$m360[1].'</div> $1 ',$content , 1); //hide zoom main image & add 360 container
                    }
...
function magiczoom_getSelectorsTemplate($info,$type) {
	...
	foreach ($images as $image) {
      ...
      if ($videoPresent) {
        ...
        $GLOBALS["magictoolbox"]['containersData'] = array();
        $videoIndex = 1;
